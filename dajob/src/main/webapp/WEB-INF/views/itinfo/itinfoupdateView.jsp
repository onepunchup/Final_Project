				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					<nav id="breadcrumbs">
						<ul>
							...
							<li style="color:#0e1f3d;">IT info Update</li>
						</ul>
					</nav>
				</div>
			</div>
					...
					<div id="parallax-testimonial-carousel"
						...
						<div class="col-md-12">
							...
							<h2 style="color: #e5fff8; line-height: 32pt;">
								...
								<form action="itupdate.do" method="post">
								<input type="hidden" name="itinfo_no" value="${itinfo.itinfo_no}">
								<input type="hidden" id="oriImg" name="itinfo_img" value="${itinfo.itinfo_img}">
								<input type="hidden" id="replaceImg" name="itinfo_img_rep" value="">
								<input type="text" name="itinfo_title" style="width:100%; height:40px; font-size:15pt;"value="${itinfo.itinfo_title}">
								<figure class="post_img">
									<a href="#">
										<img src="${pageContext.request.contextPath}/resources/images/${itinfo.itinfo_img}" style="width:100%;" alt="blog post">
									</a>
								</figure>
								<div id="fileUpload" class="dragAndDropDiv">변경할 이미지 업로드</div>
								<textarea id="editor" name="itinfo_content">${itinfo.itinfo_content}</textarea>
								<br>
								<p align="center"><button class="btn btn-success" type="submit">수정 하기</button>&nbsp;&nbsp; &nbsp;&nbsp; &nbsp;
...
<script src="${pageContext.request.contextPath}/resources/api/CKeditor/ckeditor.js"></script>
	...
	$(function() {
		...
		function handleFileUpload(files, obj) {
			var oriImg = $('#oriImg').val();
			
			for (var i = 0; i < files.length; i++) {
				var fd = new FormData();
				fd.append('oriImg', oriImg);
			...
			var jqXHR = 
				$.ajax({
						...
						success : function(data) {
							$("#replaceImg").val(decodeURIComponent(data.path.replace(/\+/g, " ")));
							alert($("#replaceImg").val());
